...
test( "Recognizing audio", function() {
    ...
    equal(segmentation.get(0).end, 15, "Value stored in Segmentation should be the correct one");
    equal(segmentation.get(1).word, "WINDOWS");
    equal(segmentation.get(1).start, 16);
    equal(segmentation.get(1).end, 61);
    equal(segmentation.get(2).word, "SUCKS");
    equal(segmentation.get(2).start, 62);
    equal(segmentation.get(2).end, 118);
    equal(segmentation.get(3).word, "<sil>");
    equal(segmentation.get(3).start, 119);
    equal(segmentation.get(3).end, 128);
    equal(segmentation.get(4).word, "AND(2)");
    equal(segmentation.get(4).start, 129);
    equal(segmentation.get(4).end, 143);
    equal(segmentation.get(5).word, "LINUX");
    equal(segmentation.get(5).start, 144);
    equal(segmentation.get(5).end, 200);
    equal(segmentation.get(6).word, "IS");
    equal(segmentation.get(6).start, 201);
    equal(segmentation.get(6).end, 213);
    equal(segmentation.get(7).word, "GREAT");
    equal(segmentation.get(7).start, 214);
    equal(segmentation.get(7).end, 272);
    equal(segmentation.get(8).word, "<sil>");
    equal(segmentation.get(8).start, 273);
    equal(segmentation.get(8).end, 294);
});
